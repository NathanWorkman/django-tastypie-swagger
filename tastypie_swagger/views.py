        ...
        if not self._tastypie_api:
            ...
            if not tastypie_api_module:
                raise ImproperlyConfigured(
                    "tastypie_api_module must be defined as an extra parameters in urls.py \
                     with its value being a path to a tastypie.api.Api instance."
                )
            if isinstance(tastypie_api_module, tastypie.api.Api):
                ...
                if not tastypie_api:
                    raise ImproperlyConfigured(
                        "%s is not a valid tastypie.api.Api instance"
                        % tastypie_api_module
                    )
...
class JSONView(TemplateView):
    ...
    def render_to_response(self, context, **response_kwargs):
        ...
        return self.response_class(
            json.dumps(context), content_type="application/json", **response_kwargs
        )
        ...
        context.update(
            {
                "basePath": self.request.build_absolute_uri(
                    reverse("%s:schema" % self.kwargs.get("namespace"))
                ).rstrip("/"),
                "apis": apis,
            }
        )
...
class SwaggerSpecs2View(TastypieApiMixin, JSONView):
    ...
    def get_context_data(self, *args, **kwargs):
        context = super(SwaggerSpecs2View, self).get_context_data(*args, **kwargs)

        # build meta specs
        title = getattr(self.tastypie_api, "title", self.tastypie_api.api_name)
        descr = getattr(self.tastypie_api, "description", self.tastypie_api.api_name)
        version = getattr(self.tastypie_api, "version", "1.0")
        basePath = getattr(self.tastypie_api, "basePath", None)
        # create output
        context.update(
            {
                "swagger": "2.0",
                "info": {"title": title, "description": descr, "version": version},
                "host": self.request.get_host(),
                "schemes": ["https" if self.request.is_secure() else "http"],
                "basePath": basePath,
